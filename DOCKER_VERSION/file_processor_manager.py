...
class FastFileProcessor(object):
    ...
    def truncate_table(self, conn, table):
        ...
        print("bulk_insert_data completed")
        cursor.close()
    def execute_batch(self, conn, df, table, page_size=1000):
        """
        ...
        query  = "INSERT INTO %s(%s) VALUES (%%s,%%s) ON CONFLICT (sku) DO UPDATE SET (sku_description) = ROW(EXCLUDED.sku_description);" % (table, cols)
        cursor = conn.cursor()
        try:
